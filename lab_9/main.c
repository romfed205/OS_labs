...
int main() {
    ...
    if (pthread_create(&writer, NULL, writerThread, NULL) != 0) {
        fprintf(stderr, "Writer creation error\n");
        return 0;
    }

    for (long i = 0; i < NUM_READERS; i++) {
        if (pthread_create(&readers[i], NULL, readerThread, (void*)i) != 0) {
            fprintf(stderr, "Reader creation error %ld\n", i);
            return 0;
        }
    }

    if (pthread_join(writer, NULL) != 0) {
        fprintf(stderr, "Writer termination error\n");
        return 0;
    }

    for (long i = 0; i < NUM_READERS; i++) {
        if (pthread_join(readers[i], NULL) != 0) {
            fprintf(stderr, "Reader termination error %ld\n", i);
            return 0;
        }
    }
